...
export default function ClientFinancialSummary({ 
  ...
  // El saldo pendiente del CRÉDITO (lo que falta por pagar del total aprobado)
  const saldoCredito = Math.max(0, montoTotal - totalPagado);
  const porcentajePagado = montoTotal > 0 ? (totalPagado / montoTotal) * 100 : 0;

  return (
    <div className="grid grid-cols-2 gap-2 sm:gap-4">
      {/* 1. Valor del Crédito */}
      <Card hasAccent className="col-span-2 !bg-white shadow-md hover:!translate-y-0 overflow-hidden relative group !p-3 sm:!p-5 border border-gray-100">
      ...
      {/* 3. Cobros Pendientes (Actionable Debt) */}
      <Card hasAccent className="col-span-1 !bg-white shadow-md hover:!translate-y-0 group border border-amber-100 relative !p-3 sm:!p-5">
        ...
        <div className="relative z-10 flex flex-col gap-1.5 sm:gap-3">
          <div className="flex items-center gap-2">
             ...
             <p className="text-[7px] sm:text-[9px] font-black uppercase tracking-wider text-gray-400 leading-tight">Cobros<br/>Pendientes</p>
          </div>
          <div>
            <p className="text-sm sm:text-lg font-black text-rose-500 tracking-tight truncate">{formatCurrency(totalDeuda)}</p>
          </div>
          <div className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-rose-50 rounded-full w-fit">
             <div className="w-1 h-1 rounded-full bg-rose-400 animate-pulse shrink-0" />
             <span className="text-[5px] sm:text-[7px] font-black text-rose-600 uppercase tracking-wider leading-none">Pagar Ahora</span>
          </div>
        </div>
      </Card>
      
      {/* 4. Pie de saldo informativo (opcional, pequeño) */}
      <div className="col-span-2 px-2 py-1">
         <p className="text-[8px] font-medium text-gray-400 uppercase tracking-[0.1em]">
           Saldo total por amortizar: <span className="font-bold text-[#0F0A4D]">{formatCurrency(saldoCredito)}</span>
         </p>
      </div>
    </div>
  );
